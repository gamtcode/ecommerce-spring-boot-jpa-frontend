...
function createForm(entity: Record<string, unknown>, operation: string): string {
    let formHTML = '';
    for (let attribute in entity) {
        if (operation === 'create' && attribute === 'id') continue;
        if ((operation === 'read' || operation === 'delete') && attribute !== 'id') continue;
        let placeholderText = '';
...
function formatResponse(data: Record<string, unknown> | Record<string, unknown>[], level = 0): string {
        ...
        return Object.entries(orderedData)
            ...
            .map(([key, value]) => {
                if (typeof value === 'object' && value !== null) {
                    return `${indent}${key.charAt(0).toUpperCase() + key.slice(1)}:<br>${formatResponse(value as Record<string, unknown>, level + 1)}`;
                } else {
...
function performOperation(entityURL: string, operation: string, entity: Record<string, unknown>): void {
    let url = entityURL;
...
function handleSelection(entitySelect: HTMLSelectElement, operationSelect: HTMLSelectElement, isEntityChange: boolean): void {
    ...
    document.getElementById('submitButton')!.onclick = () => {
        for (let attribute in entity) {
            if (operationSelect.value === 'create' && attribute === 'id') continue;
            if ((operationSelect.value === 'read' || operationSelect.value === 'delete') && attribute !== 'id') continue;
            entity[attribute] = (document.getElementById(attribute) as HTMLInputElement).value;
        }
...
document.getElementById('entitySelect')!.onchange = function () {
    handleSelection(this as HTMLSelectElement, document.getElementById('operationSelect') as HTMLSelectElement, true);
};
document.getElementById('operationSelect')!.onchange = function () {
    handleSelection(document.getElementById('entitySelect') as HTMLSelectElement, this as HTMLSelectElement, false);
};